...
- ✅ Buscador de despeses: a `/despeses`, sobre el llistat, s'ha afegit una caixa de cerca per text de la nota (amb un petit retard de 300ms mentre s'escriu, perquè no es faci una petició per cada lletra) i un desplegable per filtrar per subcategoria (agrupat per categoria, com als altres selects de l'app). Tots dos filtres es poden combinar i es mantenen en canviar de pàgina de la paginació; un botó "Neteja" els reinicia. `ExpenseController::index` aplica els filtres a la consulta (`LIKE` sobre `note`, igualtat sobre `subcategory_id`) només quan s'informen. 3 tests Pest nous (`ExpenseControllerTest.php`) i 4 tests Vitest nous (`Despeses/Index.spec.js`), tots passant.
- ✅ Pestanya "Real" a Pressupostos: la pàgina `/pressupostos` té ara dues pestanyes, **Pressupost** (la graella editable de sempre) i **Real** (només lectura). La pestanya Real mostra la despesa real amb la mateixa estructura: subcategories × 12 mesos, subtotals per categoria, fila de total general, i colors verd/vermell respecte al pressupost (només si n'hi ha). A sota, "Despesa real per banc", que agrupa pel **banc de cada despesa** (no pel banc assignat a la subcategoria, com fa el total per banc del pressupost), perquè reflecteixi d'on ha sortit realment els diners; les despeses sense banc van a "Sense banc". La pestanya activa es guarda a la URL (`?tab=real`) perquè es mantingui en canviar d'any. `BudgetController::index` envia també `bankActuals`. 1 test Pest nou i 6 tests Vitest nous, tots passant.
